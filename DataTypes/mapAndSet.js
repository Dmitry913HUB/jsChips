...
/*          Object.fromEntries: Object из Map        */
{
    ...
    alert(obj.orange); // 2
    // Вызов map.entries() возвращает итерируемый объект пар ключ/значение, 
    ...
    // Это то же самое, так как Object.fromEntries ожидает перебираемый объект в 
    // качестве аргумента, не обязательно массив. А перебор map как раз возвращает 
    // пары ключ/значение, так же, как и map.entries(). Так что в итоге у нас будет 
    // обычный объект с теми же ключами/значениями, что и в map.
...
/*          Set        */
{
    ...
    // проходит весь массив для проверки наличия элемента. Множество Set лучше 
    // оптимизировано для добавлений, оно автоматически проверяет на уникальность.
...
/*          Перебор объекта Set        */
{
    ...
    // Заметим забавную вещь. Функция в forEach у Set имеет 3 аргумента: значение value, 
    ...
    // Выглядит немного странно, но в некоторых случаях может 
    // помочь легко заменить Map на Set и наоборот.
...
/*          Итого        */
{
    ...
    // new Set(iterable) – создаёт Set, можно указать перебираемый 
